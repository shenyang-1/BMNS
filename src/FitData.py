...
class GraphFit:
    ...
    def WriteGraph(self, ob, outpath, loopNum, time, FitType="local", FitEqn="bm"):
        ...
        simoffs = linspace(offmin, offmax, int(offIncrNum))
        ...
        mData = asarray(mData, dtype=object)
        sData = array(sData)
        ...
        # ax[0,0].set_xticks(fontsize=xaxfs); ax[0,0].set_yticks(fontsize=yaxfs);
        # ax[0,1].set_xticks(fontsize=xaxfs); ax[0,1].set_yticks(fontsize=yaxfs);
        # ax[1,0].set_xticks(fontsize=xaxfs); ax[1,0].set_yticks(fontsize=yaxfs);
        # ax[1,1].set_xticks(fontsize=xaxfs); ax[1,1].set_yticks(fontsize=yaxfs);
        ax[0,0].tick_params(axis='x', labelsize=xaxfs)
        ax[0,1].tick_params(axis='x', labelsize=xaxfs)
        ax[1,0].tick_params(axis='x', labelsize=xaxfs)
        ax[1,1].tick_params(axis='x', labelsize=xaxfs)
        ax[0,0].tick_params(axis='y', labelsize=yaxfs)
        ax[0,1].tick_params(axis='y', labelsize=yaxfs)
        ax[1,0].tick_params(axis='y', labelsize=yaxfs)
        ax[1,1].tick_params(axis='y', labelsize=yaxfs)
        ...
        if len(onvals) != 0:
           # Get onres SLP min and max values and add 5%
            ...
            onIncrNum = 100
            simSLs = linspace(onmin, onmax, int(onIncrNum))
...
class Parameters:
    def __init__(self, FitNum = 0):
        ...
        self.tInc = 0.005
        self.tMax = 0.25
        self.time = linspace(0.0, self.tMax, int(self.tMax / self.tInc))
...
class Global():
    ...
    def MapGlobalP0(self):
        ...
        for x in self.keygP0:
            ...
            if par[7] is not None:
                ...
                nv = par[8]
                if par[7] == "lin":
                    bruteArry.append(linspace(lb, ub, int(nv)))
                elif par[7] == "log":
                    lb = log10(lb)
                    ub = log10(ub)
    ...
    def WriteFits(self, outPath, ob, fitnum, flag):
        ...
        if len(ob.globalFits) != 0 and flag == "global":
            ...
            if not os.path.isfile(gPath): wo = "w"
            else: wo = "a"
            with open(gPath, "a") as file:
        ...
        # Write out ob polished fit parameters in order.
            ...
            # Check to see if a file exists already
            if not os.path.isfile(pPath): wo = "w"
            else: wo = "a"
            with open(pPath, "a") as file:
                # If file exists already, don't write out header.
                if wo == "w":
                    file.write("Name,FitNum,")
                    file.write(",".join(outKeys))
        ...
        # Write out ob local fit parameters in order.
            ...
            # Check to see if a file exists already
            if not os.path.isfile(lPath): wo = "w"
            else: wo = "a"
            with open(lPath, "a") as file:
                # If file exists already, don't write out header.
                if wo == "w":
                    file.write("Name,FitNum,")
                    file.write(",".join(outKeys))
        ...
        # Write out ob MC corrupted fit parameters in order.
            ...
            # Check to see if a file exists already
            if not os.path.isfile(mcPath): wo = "w"
            else: wo = "a"
            with open(mcPath, "a") as file:
                # If file exists already, don't write out header.
                if wo == "w":
                    file.write("Name,FitNum,")
                    file.write(",".join(outKeys))
