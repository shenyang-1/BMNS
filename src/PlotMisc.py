...
def PlotBrute(args, outPath):
    ...
    if os.path.isfile(fitp):
        ...
        rd.columns = list(map(str.lower, rd.columns))
        # headers
        ...
        if len(sys.argv) >= 5:
            ...
            CS = plt.tricontourf(x,y,z, 100, cmap=plt.cm.jet)
            ...
            fig = plt.figure(figsize=(12, 8), dpi=200)
            CS = plt.tricontourf(x,y,zs, 100, cmap=plt.cm.jet)
            for c in CS.collections:
                c.set_edgecolor('none')
                c.set_linewidth(-1)
            if fitflag == "-plotbrute":
